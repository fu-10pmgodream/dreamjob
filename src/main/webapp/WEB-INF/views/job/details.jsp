            ...
            <div class="flex items-center gap-6">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-4 text-gray-600">
                        ...
                        <span class="bg-blue-50 text-blue-600 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">${job.employmentType}</span>
                    </div>
                </div>
            </div>
...
<div id="applyModal" class="fixed inset-0 bg-black bg-opacity-50 z-[100] flex items-center justify-center p-4 hidden">
    <div class="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl">
        <div class="bg-blue-600 p-6 text-white flex justify-between items-center">
            ...
            <button onclick="document.getElementById('applyModal').classList.add('hidden')" class="text-white hover:text-blue-200">
        ...
        <form action="${pageContext.request.contextPath}/jobs/apply" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            ...
            <div>
                ...
                <textarea name="coverLetter" rows="4" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="Giới thiệu ngắn gọn kinh nghiệm của bạn..."></textarea>
            </div>
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Tải lên CV (PDF/Ảnh)</label>
                <input type="file" name="cvFile" required class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
            </div>
            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl transition shadow-lg">Gửi hồ sơ ứng tuyển</button>
        </form>
    </div>
</div>
    ...
    <div class="lg:col-span-2 space-y-10">
        <section class="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
            <h2 class="text-2xl font-bold text-gray-900 mb-6 flex items-center">
                <span class="w-2 h-8 bg-blue-600 rounded-full mr-3"></span> MÔ TẢ CÔNG VIỆC
            </h2>
        ...
        <section class="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
            <h2 class="text-2xl font-bold text-gray-900 mb-6 flex items-center">
                <span class="w-2 h-8 bg-blue-600 rounded-full mr-3"></span> YÊU CẦU CÔNG VIỆC
            </h2>
            ...
            <div class="space-y-4">
                <c:forEach items="${similarJobs}" var="sj">
                    <div class="bg-white p-4 rounded-2xl border border-gray-50 hover:shadow-md transition cursor-pointer" onclick="location.href='${pageContext.request.contextPath}/jobs/${sj.jobId}'">
                        ...
                        <p class="text-sm text-blue-600 mb-2">${sj.companyName}</p>
